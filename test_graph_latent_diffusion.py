...
def test_graph_latent_diffusion(config):
    print("start test_graph_latent_diffusion pipeline ...")
    seeding(config['seed'])
    ...
    with torch.no_grad():
        for batch in tqdm(test_loader):
            ...
            for ix in range(test_config['sample_num']):
                pred_x = sampler_fn(config, ema.model, sde, batch['cond_x'], batch['cond_e'], batch['mask'], mode=mode).squeeze(0).cpu()
                pred_x[pred_x < 0] = 0
